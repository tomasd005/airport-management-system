...
#include <string.h>
#include <time.h>
#include <stdio.h> // <-- FALTA ESTE

struct gestor_reservas
{
    GArray *reservas;

    // flight_id -> número de passageiros (contador simples)
    GHashTable *contador_passageiros_por_voo;
};
...
gestor_reservas_t *gestor_reservas_criar(void)
{
    ...
    g->contador_passageiros_por_voo = g_hash_table_new_full(
        g_str_hash,
        g_str_equal,
        g_free,
        g_free);
...
void gestor_reservas_destruir(gestor_reservas_t *gestor)
{
    ...
    g_hash_table_destroy(gestor->contador_passageiros_por_voo);
    free(gestor);
}
...
void gestor_reservas_adicionar(gestor_reservas_t *gestor, reserva_t *r)
{
    ...
    for (size_t i = 0; i < num_voos; i++)
    {
        ...
        guint *contador = g_hash_table_lookup(
            gestor->contador_passageiros_por_voo,
            flight_id);

        if (!contador)
        {
            contador = g_malloc(sizeof(guint));
            *contador = 0;
            g_hash_table_insert(
                gestor->contador_passageiros_por_voo,
                g_strdup(flight_id),
